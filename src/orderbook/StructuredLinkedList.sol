...
library StructuredLinkedList {
     ...
     * @param _bGt true then value of the node less than _value, otherwise greater than _value
     * @return uint256 next node with a value less/greater than _value
     */
    function getSortedSpot(List storage self, address _structure, uint256 _value, uint256 nearestNode, bool _bGt) internal view returns (uint256) {
        if (sizeOf(self) == 0) {
            return 0;
        }

        uint256 next;
        (, next) = getAdjacent(self, nearestNode, _NEXT);
        if (_bGt) {
