...
// File: blast_inscription/IBlast.sol
...
// File: blast_inscription/BlastClaim.sol
...
// File: blast_inscription/SmartBlas.sol
...
contract SmartBlas is BlastClaim, ERC20 {
    ...
    function initialize(string memory _inscriptionName, uint256 fee, uint256 limit, uint256 maxMint) public {        
...
// File: blast_inscription/DeployFactory.sol
...
contract DeployFactory is BlastClaim {
    ...
    constructor() BlastClaim(msg.sender) {
    }
    ...
    function deploy(string memory name, uint256 fee, uint256 limit, uint256 maxMints) public payable {
        require(msg.value >= deployFee, "Deploy: fee not enough");
        require(bytes(name).length == 4, "length of name != 4");
        require(nameDeployInfoMap[name].inscription == address(0), "Exist!");

        SmartBlas blas = new SmartBlas(msg.sender);                
        blas.initialize(name, fee, limit, maxMints);

        nameDeployInfoMap[name] = DeployInfo(name, fee, limit, maxMints, address(blas));
        nameList.push(name);
        ...
        emit DEPOLY(msg.sender, deployInscription);
    }
    ...
    function getAllInscriptions() view public 
        returns(DeployInfo[] memory deployInfos) {
        deployInfos = new DeployInfo[](nameList.length);

        for (uint256 i = 0; i < nameList.length; i++) {
            deployInfos[i] = nameDeployInfoMap[nameList[i]];
        }
    }
